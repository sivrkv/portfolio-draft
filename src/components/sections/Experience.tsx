  ...
  return (
    <section id="experience" className="py-24 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Vertical Timeline */}
        <div className="relative border-l-2 border-[var(--border-color)] ml-4 md:ml-8 space-y-12 pl-6 md:pl-10">
          {EXPERIENCE_DATA.map((item, idx) => (
            ...
            >
              ...
              <div className="absolute -left-[31px] md:-left-[47px] top-1.5 w-6 h-6 rounded-full bg-[var(--bg-dark)] border-2 border-cyan-400 flex items-center justify-center group-hover:scale-125 transition-transform duration-200 shadow-md shadow-cyan-500/20">
              ...
              {item.isCondensed ? (
                ...
                <GlassCard className="p-4 md:p-6">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                    ...
                    <span className="text-xs font-mono-tech px-2.5 py-1 rounded-md bg-[var(--bg-card)] text-[var(--text-secondary)] border border-[var(--border-color)]">
                      {item.period}
                    </span>
                  </div>
                ...
                <GlassCard className="p-6 md:p-8 border-l-4 border-l-cyan-400">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-sm text-cyan-400 font-semibold font-mono-tech">
                        ...
                        {item.location && (
                          <span className="flex items-center text-[var(--text-secondary)] text-xs font-normal">
                  ...
                  <ul className="space-y-2.5 mb-6">
                    {item.bullets.map((bullet, bulletIdx) => (
                      <li
                        key={bulletIdx}
                        className="flex items-start text-sm text-[var(--text-secondary)] leading-relaxed"
                      >
                  ...
                  {item.metrics && item.metrics.length > 0 && (
                    <div className="pt-4 border-t border-[var(--border-color)]/80 mb-5">
                      <div className="text-xs uppercase font-mono-tech text-[var(--text-secondary)] mb-3 tracking-wider flex items-center space-x-1.5">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {item.metrics.map((metric, mIdx) => (
                          <div
                            key={mIdx}
                            className="bg-[var(--bg-card)]/90 border border-[var(--border-color)] rounded-xl p-3.5 text-center"
                          >
